...
if st.session_state.show_home_modal:
    ...
    # with open(r'ui\range_config.json', 'r') as f:
    #         range_config = json.load(f)

    # with open(r'ui\tabnet_classifcation_range_config.json', 'r') as f:
    #         tabnet_classification_range_config = json.load(f)
    ...
    if input_role=='Highway Patrol Officer':
        ...
        if hpo_county_mode=="List Selection":
                ...
                if tract_mode == "Single Census Tract":
                    ...
                    # first_feature = geojson_data["features"]
                    # # print(geojson_data.to) 
                    # # Save the full GeoJSON to a file
                    # with open("full_geojson_data.geojson", "w+") as f:
                    #     json.dump(geojson_data, f, indent=4)
                    #             # usually ['type', 'properties', 'geometry']
